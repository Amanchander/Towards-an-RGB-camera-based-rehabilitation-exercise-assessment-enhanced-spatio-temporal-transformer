...
train_x = joblib.load(X_train_path)

y_train_path = path + '/Ex_' + current_exercise + '/train_score.pkl' 
train_y = joblib.load(y_train_path)


X_test_path = path + '/Ex_' + current_exercise + '/test_data.pkl' 
valid_x = joblib.load(X_test_path)

y_test_path = path + '/Ex_' + current_exercise + '/test_score.pkl' 
valid_y = joblib.load(y_test_path)
...
nan_mask_train_X = np.isnan(train_x)
mean_values_train_X = np.nanmean(train_x) 
train_x[nan_mask_train_X] = mean_values_train_X

nan_mask_train_y = np.isnan(train_y)
mean_values_train_y = np.nanmean(train_y)
train_y[nan_mask_train_y] = mean_values_train_y

# Replace NaN values with mean in testing data
nan_mask_test_X = np.isnan(valid_x)
mean_values_test_X = np.nanmean(valid_x)
valid_x[nan_mask_test_X] = mean_values_test_X

nan_mask_test_y = np.isnan(valid_y)
mean_values_test_y = np.nanmean(valid_y)
valid_y[nan_mask_test_y] = mean_values_test_y
...
def reshapeMyData(in_data):
    data_shape = in_data.shape
    new_data = in_data.reshape(data_shape[0], data_shape[1], data_shape[2] * data_shape[3])
    return new_data


X_train = reshapeMyData(train_x)
X_test  = reshapeMyData(valid_x)

y_train = torch.sigmoid(train_y)
y_test  = torch.sigmoid(valid_y)
...
def makeFrames(x, frame_size, stride):  
    ...
    # print("The size of x after appending is ", x.shape)
...
class mySpatioTemporalTransformer(nn.Module):
    def __init__(self, input_dim, model_dim, dim_head, frame_size, no_of_frames, num_heads, num_layers, dim_feedforward, stride, dropout, dense_layer_neurons):
        ...
        # self.dense_1                        = nn.Linear(8496 , dense_layer_neurons)  # Adjust units as needed
        self.relu                           = nn.ReLU()
...
def trainMyModel(model, train_data, val_data, epochs):
    ...
    for epoch in range(epochs):
        ...
        for val, target in zip(X_train_val, y_train_val):
            ...
            # Gradient clipping
            torch_utils.clip_grad_norm_(model.parameters(), max_norm=1.0)  # Adjust max_norm as needed
        ...
        print(f"Epoch [{epoch + 1}/{epochs}], Loss: {train_loss:.4f}    Val loss: {train_val_loss:.4f}    Best model at epoch: {desired_epoch_number} ")
    ...
    print("The best model was found at epoch number ", desired_epoch_number)
...
def evaluateMyModel(model, test_data, test_labels):    
    ...
    with torch.no_grad():
        ...
        print("For exercise :    ", current_exercise)
...
model_dim = 8 # Associated with the capturing of different features
num_heads = 4
dim_feedforward = 8
num_layers = 6
dropout = 0.2
...
all_frames = makeDataDivisibleBy8(all_frames)
