...
exercise = '3';
iteration_no_to_ignore = 3; % This is used to ignore the subject data which is not available otherwise it will give error. Set to some number more than total subjects if you don't want to ignore any data point


% Load the correct angle movements for DeepSquat
% To read text files use: M = csvread('m01_s01_e02_angles.txt');
for i=1:nSubjects-1
    if i ~= iteration_no_to_ignore
        for j=1:nEpisodes-1
            eval(['Train1_ang_cor{',int2str(i),',',int2str(j),'} = csvread(''Segmented Movements\Vicon\Angles\m0',exercise,'_s0',int2str(i),'_e0',int2str(j),'_angles.txt'');']);
        end
    end
end
...
for i=1:nSubjects-1
    for j=nEpisodes
         if i ~= iteration_no_to_ignore
            eval(['Train1_ang_cor{',int2str(i),',',int2str(j),'} = csvread(''Segmented Movements\Vicon\Angles\m0',exercise,'_s0',int2str(i),'_e',int2str(j),'_angles.txt'');']);
         end
...
for i=nSubjects
    for j=1:nEpisodes-1
        if i ~= iteration_no_to_ignore
            eval(['Train1_ang_cor{',int2str(i),',',int2str(j),'} = csvread(''Segmented Movements\Vicon\Angles\m0',exercise,'_s',int2str(i),'_e0',int2str(j),'_angles.txt'');']);
        end
    end
end
...
for i=nSubjects
    for j=nEpisodes
        if i ~= iteration_no_to_ignore
            eval(['Train1_ang_cor{',int2str(i),',',int2str(j),'} = csvread(''Segmented Movements\Vicon\Angles\m0',exercise,'_s',int2str(i),'_e',int2str(j),'_angles.txt'');']);
        end
    end
end
...
for i=1:nSubjects-1
    for j=1:nEpisodes-1
        if i ~= iteration_no_to_ignore
            eval(['Test1_ang_inc{',int2str(i),',',int2str(j),'} = csvread(''Incorrect Segmented Movements\Vicon\Angles/m0', exercise, '_s0',int2str(i),'_e0',int2str(j),'_angles_inc.txt'');']);
        end
    end
end
...
for i=1:nSubjects-1
    for j=nEpisodes
        if i ~= iteration_no_to_ignore
           eval(['Test1_ang_inc{',int2str(i),',',int2str(j),'} = csvread(''Incorrect Segmented Movements\Vicon\Angles/m0', exercise, '_s0',int2str(i),'_e',int2str(j),'_angles_inc.txt'');']);
        end
    end
end
...
for i=nSubjects
    for j=1:nEpisodes-1
        if i ~= iteration_no_to_ignore
           eval(['Test1_ang_inc{',int2str(i),',',int2str(j),'} = csvread(''Incorrect Segmented Movements\Vicon\Angles/m0', exercise, '_s',int2str(i),'_e0',int2str(j),'_angles_inc.txt'');']);
        end
    end
end
...
for i=nSubjects
    for j=nEpisodes
        eval(['Test1_ang_inc{',int2str(i),',',int2str(j),'} = csvread(''Incorrect Segmented Movements\Vicon\Angles/m0', exercise, '_s',int2str(i),'_e',int2str(j),'_angles_inc.txt'');']);
    end
end
...
for i = 1:nSequences
    if size(Correct_Red{i}, 1) >= 2
        for j = 1:nDim
            Correct_Aligned{i}(:,j) = interp1([1:size(Correct_Red{i},1)], Correct_Red{i}(:,j), linspace(1, size(Correct_Red{i},1), Length_mean));
        end
    else
        % Handle cases where the sequence has less than 2 points
        % You may choose to skip these sequences or handle them differently
        Correct_Aligned{i} = nan(Length_mean, nDim);
    end
end

% Incorrect sequences
for i = 1:nSequences
    if size(Incorrect_Red{i}, 1) >= 2
        for j = 1:nDim
        ...
        % Handle cases where the sequence has less than 2 points
        % You may choose to skip these sequences or handle them differently
        Incorrect_Aligned{i} = nan(Length_mean, nDim);
    end
end
...
folder_path = % path to save 'Data_Correct.csv' and 'Data_Incorrect.csv' %

csvwrite([folder_path, exercise,'\Data_Correct.csv'],Data_Correct)
csvwrite([folder_path, exercise,'\Data_Incorrect.csv'],Data_Incorrect)
