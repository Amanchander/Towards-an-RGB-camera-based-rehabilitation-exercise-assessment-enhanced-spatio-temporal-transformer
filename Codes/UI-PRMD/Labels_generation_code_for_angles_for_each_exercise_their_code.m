...
folder_path = % path to the Vicon data files 

exercise = 'm03';
addpath([folder_path, exercise, '\'])
path_to_save_results = [folder_path, exercise];
...
print(h,[path_to_save_results, 'GMM_Encoded_Movements'],'-dpng','-r300');
...
print(h,[path_to_save_results, 'GMM_Loglikelihood_Scores'],'-dpng','-r300');
...
set(gca,'box','off','fontweight','bold','LineWidth',2);
set(gcf,'Units','inches','position',[0 0 5.5 4.5]);
print(h,[path_to_save_results, 'GMM_Movement_Quality_Scores'],'-dpng','-r300');

%% Save the labels

csvwrite([path_to_save_results, '\Labels_Correct.csv'],Correct_Y')
csvwrite([path_to_save_results, '\Labels_Incorrect.csv'],Incorrect_Y')
